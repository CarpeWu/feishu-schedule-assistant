...
## **第二部分：集成的十一诫 (The Eleven Commandments of Integration)**

此十一诫是所有设计与开发工作的最高行为准则。
...
*   **第十一诫：异步优先，拒绝超时 (Async First, No Timeout)**
    *   飞书 Webhook 等待响应的阈值极短（约3秒）。对于逻辑复杂或 API 调用较多的工作流（如 WF-04），必须在通过 Challenge 校验后，**立即连接 `Respond to Webhook` 节点返回 200 OK**，然后再处理后续业务逻辑。严禁让飞书服务器同步等待 n8n 跑完整个流程，这会导致“幽灵重复”问题。
...
*   **正确范式**: **Switch 路由分发** 模式。
    1.  在 `Webhook` 节点的**正后方**，**必须**立即放置一个 `Respond to Webhook` (Early Return) 节点防止超时。
    2.  随后连接 `Switch` 节点，根据 `table_id` 进行多路分发。
    3.  所有核心的业务逻辑，都应该连接到该 `Switch` 节点对应的 Output 分支。
    4.  未匹配的分支（默认输出）不连接任何节点。
...
*   **正确范式**:
    1.  在 Webhook 监听事件后，通过 `Switch` 节点路由到 `T5` 分支。
...
#### **陷阱 1.6：Webhook 超时导致的幽灵重复 (Ghost Retries)**
*   **错误症状**: 逻辑只触发了一次，但飞书发起了多次 Webhook 请求（`x-request-id` 相同，间隔 20s/60s），导致消息重复发送。
*   **错误根源**: 工作流执行耗时超过 3 秒，飞书判定超时并重试。
*   **正确范式**: **Fire and Forget** 模式。
    `Webhook` -> `IF (Verify)` -> `Respond to Webhook (200 OK)` -> `Switch/Code (Business Logic)`。
    确保飞书服务器收到 200 OK 后，n8n 继续在后台执行耗时逻辑。
...
#### **陷阱 4.3：过滤与分裂的模式选择**
*   **错误症状**: Code 节点用于过滤（返回空数组 `[]` 拦截流程）或分裂（1个配置 -> N个任务）时，n8n 报错 "Code doesn't return a single object" 或者流程未能按预期分裂。
*   **错误根源**: 使用了默认的 `Run Once for Each Item` 模式，该模式期望输入输出 1:1 映射。
*   **正确范式**: 当 Code 节点的输出数量可能与输入数量不一致（0个或 N个）时，**必须**将 Mode 设置为 **`Run Once for All Items`**。
    ```javascript
    // 示例：返回空数组以停止流程
    return []; 
...
### **工作流逻辑**
- [ ] **异步响应**: Webhook 后是否立即连接了 `Respond to Webhook` (200 OK)？
- [ ] **路由分发**: 是否使用 `Switch` 节点根据 `table_id` 分发逻辑？
- [ ] **Code模式**: 过滤/分裂逻辑的 Code 节点是否开启了 `Run Once for All Items`？
...
*   **v1.3.0 (2025-12-11)**
    *   **[新增核心原则]** 新增“第十一诫：异步优先，拒绝超时”，明确了 Fire and Forget 模式以解决 Webhook 重复触发问题。
    *   **[新增陷阱]** 记录了“陷阱 1.6：Webhook 超时导致的幽灵重复”。
    *   **[新增陷阱]** 记录了“陷阱 4.3：过滤与分裂的模式选择”，规范了 Code 节点的 Mode 配置。
    *   **[更新范式]** 将 WF-04 的推荐架构从 IF 级联更新为 Switch 路由。
*   **v1.2.0 (2025-11-28)**
    *   **[新增规范]** 在“工作流设计”领域新增了 **“陷阱 1.5：T5 员工表人员字段自动回填”**。
    *   **[API 操作补充]** 明确了 `bitable:table:record:get` 和 `bitable:table:record:update` 的使用场景。
*   **v1.1.0 (2025-11-10)**
    *   **[新增规范]** 在“工作流设计”领域新增了 **“陷阱 1.4：Webhook噪音”**。
*   **v1.0.0 (2025-11-09)**
    *   项目首个稳定版本发布。
